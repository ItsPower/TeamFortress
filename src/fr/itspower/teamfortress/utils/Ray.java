...
public class Ray {
    ...
    public static Ray from(Location loc) {
        return new Ray(loc.toVector(), loc.getDirection());
    }
    
    public Ray changeDir(double accu) {
    	//Bukkit.broadcastMessage("§a"+direction.getX()+" "+direction.getY()+" "+direction.getZ()+" ");
    	double x = direction.getX() + (new Random().nextGaussian()*2-1) * accu;
    	double y = direction.getY() + (new Random().nextGaussian()*2-1) * accu;
    	double z = direction.getZ() + (new Random().nextGaussian()*2-1) * accu;
    	//Bukkit.broadcastMessage("§2"+x+" "+y+" "+z+" ");
		return new Ray(origin, new Vector(x,y,z));
    }
